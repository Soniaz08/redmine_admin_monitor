module AdminMonitorAlertsHelper
	...
	def toggel_silent(alert)
		if alert.silent_flag
			return link_to image_tag("silent.png", :plugin => "redmine_admin_monitor", :title => "Click to enable mail alert for this error") , {:remote => true , :controller => :admin_monitor_alerts , :action => :silent , :id => alert.id , :silent_flag => false } , {:id => "admin_monitor_alerts_silent_flag_#{alert.id}"  ,:class => "admin_monitor_alerts_silent_flag"}							
		else	
			return link_to image_tag("unsilent.png", :plugin => "redmine_admin_monitor", :title => "Click to disable mail alert for this error") , {:remote => true , :controller => :admin_monitor_alerts , :action => :silent , :id => alert.id , :silent_flag => true } , {:id => "admin_monitor_alerts_silent_flag_#{alert.id}"  ,:class => "admin_monitor_alerts_silent_flag"}	
		end	
	end	
